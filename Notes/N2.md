## Lecture 2: Functions, Graphs, and Transformations
...
scale by $a$ (the slope) followed by a shift of $b$ (the intercept). 

**Exercise 7.** Find the line through $(-1,4)$ and $(3,-8)$.

**Quadratic functions.** Every quadratic is a transformation of the parent
graph $y=x^2$, built up one step at a time from the table above:

1. Start with $y=x^2$, vertex $(0,0)$.
2. Scale vertically by $a$: $y=ax^2$. The vertex stays at $(0,0)$; the
 parabola narrows if $|a|>1$, widens if $|a|<1$, and flips upside down if
 $a<0$.
3. Shift right $h$: $y=a(x-h)^2$. The vertex moves to $(h,0)$.
4. Shift up $k$: $y=a(x-h)^2+k$. The vertex moves to $(h,k)$.

None of these steps change the parabola's shape, so $y=a(x-h)^2+k$ is
always a parabola with vertex $(h,k)$, opening upward if $a>0$ and downward
if $a<0$.

A quadratic given in standard form $ax^2+bx+c$ is the same parabola, just
not yet written to show $h$ and $k$. **Completing the square** finds them:
factor $a$ out of the first two terms, then add and subtract the constant
that completes the square inside the parentheses.
$$
ax^2+bx+c
...
**Example.** For $2x^2-8x+3$, $a=2$ and $b=-8$, so
$h=-\dfrac{-8}{2(2)}=2$ and $k=3-\dfrac{(-8)^2}{4(2)}=3-8=-5$. Thus
$2x^2-8x+3=2(x-2)^2-5$, vertex $(2,-5)$ — matching the four-step
construction: start from $x^2$, scale by $2$ (narrower, still opening up),
shift right $2$, shift down $5$.
...
* $y=1/x$ is decreasing on $(-\infty,0)$ and decreasing on $(0,\infty)$ —
 but, as Exercise 15 shows, it is *not* decreasing on its full domain.
